...
RUNTIME_BASE="${RUNTIME_BASE:-/idfsdata/yexuyan/AgentGym-RL/.runtime}"
...
tmux new-session -d -s "${ENV_SESSION}" \
  "cd ${ROOT} && TMPDIR=${TMPDIR} TMP=${TMP} TEMP=${TEMP} XDG_CACHE_HOME=${XDG_CACHE_HOME} HF_HOME=${HF_HOME} TRANSFORMERS_CACHE=${TRANSFORMERS_CACHE} WANDB_DIR=${WANDB_DIR} WANDB_CACHE_DIR=${WANDB_CACHE_DIR} WANDB_CONFIG_DIR=${WANDB_CONFIG_DIR} HOST=127.0.0.1 PORT=${ENV_PORT} LOG_PATH=${ENV_LOG} bash ${ROOT}/scripts/run_webshop_env_service.sh"
...
tmux new-session -d -s "${TRAIN_SESSION}" \
  "cd ${ROOT} && TMPDIR=${TMPDIR} TMP=${TMP} TEMP=${TEMP} RAY_TMPDIR=${RAY_TMPDIR} XDG_CACHE_HOME=${XDG_CACHE_HOME} HF_HOME=${HF_HOME} TRANSFORMERS_CACHE=${TRANSFORMERS_CACHE} WANDB_DIR=${WANDB_DIR} WANDB_CACHE_DIR=${WANDB_CACHE_DIR} WANDB_CONFIG_DIR=${WANDB_CONFIG_DIR} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} ENV_ADDR=${ENV_ADDR} MODEL_PATH=${MODEL_PATH} WANDB_MODE=${WANDB_MODE} WANDB_BASE_URL=${WANDB_BASE_URL} WANDB_ANONYMOUS=${WANDB_ANONYMOUS} WANDB_ENTITY=${WANDB_ENTITY} PROJECT_NAME=${PROJECT_NAME} ROLLOUT_N=${ROLLOUT_N} TOTAL_EPOCHS=${TOTAL_EPOCHS} SAVE_FREQ=${SAVE_FREQ} ENABLE_WMC=${ENABLE_WMC} WMC_COEFF=${WMC_COEFF} ENABLE_ERC=${ENABLE_ERC} ERC_MU_BASE=${ERC_MU_BASE} ERC_MU_EXP=${ERC_MU_EXP} ERC_ETA_WM=${ERC_ETA_WM} ERC_LAMBDA_WM=${ERC_LAMBDA_WM} ERC_CLIPPING_TYPE=${ERC_CLIPPING_TYPE} ERC_CLIPPING_METHOD=${ERC_CLIPPING_METHOD} ERC_MOMENTUM=${ERC_MOMENTUM} REWARD_MODE=${REWARD_MODE} ORM_SUCCESS_SCORE=${ORM_SUCCESS_SCORE} REMOVE_PREVIOUS_CKPT_IN_SAVE=${REMOVE_PREVIOUS_CKPT_IN_SAVE} MAX_LOCAL_CKPT_TO_KEEP=${MAX_LOCAL_CKPT_TO_KEEP} EXP_NAME=${EXP_NAME} LOG_PATH=${TRAIN_LOG} bash ${ROOT}/scripts/run_webshop_grpo_train.sh"
