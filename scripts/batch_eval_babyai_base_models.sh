...
BASE_3B_MODEL_PATH="${BASE_3B_MODEL_PATH:-${ROOT}/models/Qwen2.5-3B-Instruct}"
BASE_7B_MODEL_PATH="${BASE_7B_MODEL_PATH:-${ROOT}/models/Qwen2.5-7B-Instruct}"
...
candidate_lines_for_model() {
  local model_label="$1"
  if [[ "${model_label}" == "base_3b" ]]; then
    printf '%s\n' \
      "96:0.88:384:49152" \
      "64:0.86:256:32768" \
      "48:0.84:192:24576" \
...
run_one_model_with_fallback() {
  ...
  done < <(candidate_lines_for_model "${model_label}")
...
MODEL_LABELS=("base_3b" "base_7b")
MODEL_PATHS=("${BASE_3B_MODEL_PATH}" "${BASE_7B_MODEL_PATH}")
...
echo "All BabyAI base model evaluations completed."
echo "Results file: ${RESULTS_FILE}"
