#!/usr/bin/env bash

set -euo pipefail

ROOT="${ROOT:-/idfsdata/yexuyan/AgentGym-RL}"
CODE_DIR="${ROOT}/AgentGym-RL"
CONDA_SH="${CONDA_SH:-/home/yexuyan/miniconda3/etc/profile.d/conda.sh}"
TRAIN_ENV="${TRAIN_ENV:-/idfsdata/yexuyan/conda_envs/agentgym-rl-webshop}"

ENV_ADDR="${ENV_ADDR:-http://127.0.0.1:8015}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
RESULTS_FILE="${RESULTS_FILE:-${ROOT}/FinalResults_WebShop_3B7B_8GPU.jsonl}"
OVERWRITE_RESULTS="${OVERWRITE_RESULTS:-1}"
SKIP_DONE_MODELS="${SKIP_DONE_MODELS:-1}"
INCLUDE_BASE_MODELS="${INCLUDE_BASE_MODELS:-1}"

BASE_MODEL_3B="${BASE_MODEL_3B:-${ROOT}/models/Qwen2.5-3B-Instruct}"
BASE_MODEL_7B="${BASE_MODEL_7B:-${ROOT}/models/Qwen2.5-7B-Instruct}"
CKPT_ROOT_3B="${CKPT_ROOT_3B:-${ROOT}/checkpoints/WEBSHOP_GRPO_3B_SCORE_SF50_20260418_171148}"
CKPT_ROOT_7B="${CKPT_ROOT_7B:-${ROOT}/checkpoints/WEBSHOP_GRPO_7B_SCORE_20260418_055839}"

TEST_DATA_DIR="${TEST_DATA_DIR:-${ROOT}/AgentItemId/test}"
MAX_ROUNDS="${MAX_ROUNDS:-15}"
TIMEOUT_SECONDS="${TIMEOUT_SECONDS:-2400}"
MAX_PROMPT_LENGTH="${MAX_PROMPT_LENGTH:-768}"
MAX_RESPONSE_LENGTH="${MAX_RESPONSE_LENGTH:-8192}"
MAX_MODEL_LEN="${MAX_MODEL_LEN:-16384}"
MAX_TOKENS_PER_TURN="${MAX_TOKENS_PER_TURN:-256}"
N_SAMPLES="${N_SAMPLES:-1}"

# High-concurrency defaults on 8 GPUs.
EVAL_BATCH_SIZE_3B="${EVAL_BATCH_SIZE_3B:-160}"
EVAL_BATCH_SIZE_7B="${EVAL_BATCH_SIZE_7B:-96}"

if [[ "${OVERWRITE_RESULTS}" == "1" ]]; then
  : > "${RESULTS_FILE}"
fi

mkdir -p "$(dirname "${RESULTS_FILE}")"

normalize_root_path() {
  local raw="$1"
  if [[ -d "${raw}" ]]; then
    echo "${raw}"
    return 0
  fi
  local fixed="${raw/AgentGentGym-RL/AgentGym-RL}"
  echo "${fixed}"
}

CKPT_ROOT_3B="$(normalize_root_path "${CKPT_ROOT_3B}")"
CKPT_ROOT_7B="$(normalize_root_path "${CKPT_ROOT_7B}")"

if [[ ! -d "${CKPT_ROOT_3B}" ]]; then
  echo "3B checkpoint root not found: ${CKPT_ROOT_3B}" >&2
  exit 1
fi
if [[ ! -d "${CKPT_ROOT_7B}" ]]; then
  echo "7B checkpoint root not found: ${CKPT_ROOT_7B}" >&2
  exit 1
fi

source "${CONDA_SH}"
set +u
conda activate "${TRAIN_ENV}"
set -u

python "${ROOT}/scripts/prepare_webshop_grpo_splits.py"

if [[ ! -f "${TEST_DATA_DIR}/webshop_test.json" ]]; then
  echo "Missing WebShop test file: ${TEST_DATA_DIR}/webshop_test.json" >&2
  exit 1
fi

export NO_PROXY="${NO_PROXY:-127.0.0.1,localhost}"
export no_proxy="${no_proxy:-127.0.0.1,localhost}"

IFS=',' read -r -a GPU_ARRAY <<< "${CUDA_VISIBLE_DEVICES}"
NUM_GPUS="${#GPU_ARRAY[@]}"
if (( NUM_GPUS < 1 )); then
  echo "CUDA_VISIBLE_DEVICES is empty." >&2
  exit 1
fi

if (( EVAL_BATCH_SIZE_3B < NUM_GPUS )); then
  EVAL_BATCH_SIZE_3B="${NUM_GPUS}"
fi
if (( EVAL_BATCH_SIZE_7B < NUM_GPUS )); then
  EVAL_BATCH_SIZE_7B="${NUM_GPUS}"
fi
if (( EVAL_BATCH_SIZE_3B % NUM_GPUS != 0 )); then
  EVAL_BATCH_SIZE_3B="$(( (EVAL_BATCH_SIZE_3B / NUM_GPUS + 1) * NUM_GPUS ))"
fi
if (( EVAL_BATCH_SIZE_7B % NUM_GPUS != 0 )); then
  EVAL_BATCH_SIZE_7B="$(( (EVAL_BATCH_SIZE_7B / NUM_GPUS + 1) * NUM_GPUS ))"
fi

echo "Using ${NUM_GPUS} GPUs (${CUDA_VISIBLE_DEVICES})"
echo "EVAL_BATCH_SIZE_3B=${EVAL_BATCH_SIZE_3B}, EVAL_BATCH_SIZE_7B=${EVAL_BATCH_SIZE_7B}"

has_hf_weights() {
  local model_path="$1"
  [[ -f "${model_path}/model.safetensors.index.json" || -f "${model_path}/model.safetensors" || -f "${model_path}/pytorch_model.bin" ]]
}

ensure_model_weights() {
  local model_label="$1"
  local model_path="$2"
  local merge_log="$3"

  if has_hf_weights "${model_path}"; then
    echo "[merge] ${model_label}: huggingface weights already present."
    return 0
  fi

  local actor_dir
  actor_dir="$(dirname "${model_path}")"
  if ! compgen -G "${actor_dir}/model_world_size_*_rank_0.pt" > /dev/null; then
    echo "[merge] ${model_label}: missing shard files under ${actor_dir}" | tee -a "${merge_log}"
    return 1
  fi

  echo "[merge] ${model_label}: start merging shards from ${actor_dir}" | tee -a "${merge_log}"
  if (
    cd "${CODE_DIR}"
    python scripts/model_merger.py --local_dir "${actor_dir}"
  ) >> "${merge_log}" 2>&1; then
    if has_hf_weights "${model_path}"; then
      echo "[merge] ${model_label}: merge success" | tee -a "${merge_log}"
      return 0
    fi
  fi

  echo "[merge] ${model_label}: merge failed (see ${merge_log})" | tee -a "${merge_log}"
  return 1
}

should_skip_model() {
  local model_label="$1"
  local results_file="$2"
  if [[ "${SKIP_DONE_MODELS}" != "1" || ! -s "${results_file}" ]]; then
    return 1
  fi
  python - "${results_file}" "${model_label}" <<'PY'
import json
import sys
path, target = sys.argv[1], sys.argv[2]
for raw in open(path, "r", encoding="utf-8"):
    line = raw.strip()
    if not line:
        continue
    try:
        row = json.loads(line)
    except Exception:
        continue
    if row.get("model_label") == target:
        print("1")
        break
else:
    print("0")
PY
}

is_7b_model() {
  local label="$1"
  local path="$2"
  local low="${label,,} ${path,,}"
  [[ "${low}" == *"7b"* ]]
}

is_oom_log() {
  local log_file="$1"
  grep -Eiq \
    "cuda out of memory|outofmemoryerror|no available memory for the cache blocks|oom|allocation failed|device out of memory" \
    "${log_file}"
}

run_one_model_with_fallback() {
  local model_label="$1"
  local model_path="$2"
  local run_dir="$3"
  local log_path="$4"

  local -a candidates=()
  if is_7b_model "${model_label}" "${model_path}"; then
    candidates=(
      "${EVAL_BATCH_SIZE_7B}:0.88:512:49152"
      "80:0.86:448:40960"
      "64:0.84:384:32768"
      "48:0.82:320:24576"
      "32:0.80:256:20480"
    )
  else
    candidates=(
      "${EVAL_BATCH_SIZE_3B}:0.92:768:65536"
      "128:0.90:640:57344"
      "96:0.88:512:49152"
      "80:0.86:448:40960"
      "64:0.84:384:32768"
    )
  fi

  local ok=1
  local attempt=0
  for c in "${candidates[@]}"; do
    attempt=$((attempt + 1))
    IFS=':' read -r batch_size gpu_util max_num_seqs max_num_batched_tokens <<< "${c}"
    local attempt_log="${run_dir}/eval_attempt_${attempt}.log"

    echo "===== ATTEMPT ${attempt} model=${model_label} batch=${batch_size} util=${gpu_util} max_num_seqs=${max_num_seqs} max_num_batched_tokens=${max_num_batched_tokens} =====" | tee -a "${log_path}"

    (
      cd "${CODE_DIR}"
      exec env \
        -u http_proxy -u https_proxy -u all_proxy \
        -u HTTP_PROXY -u HTTPS_PROXY -u ALL_PROXY \
        NO_PROXY="${NO_PROXY}" \
        no_proxy="${no_proxy}" \
        CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
        VLLM_USE_MODELSCOPE=0 \
        VLLM_WORKER_MULTIPROC_METHOD=spawn \
        VLLM_ATTENTION_BACKEND=XFORMERS \
        HYDRA_FULL_ERROR=1 \
        python -m verl.agent_trainer.main_generation \
          data.path="${TEST_DATA_DIR}" \
          data.max_prompt_length="${MAX_PROMPT_LENGTH}" \
          data.max_response_length="${MAX_RESPONSE_LENGTH}" \
          data.n_samples="${N_SAMPLES}" \
          data.batch_size="${batch_size}" \
          agentgym.task_name=webshop \
          agentgym.env_addr="${ENV_ADDR}" \
          agentgym.max_rounds="${MAX_ROUNDS}" \
          agentgym.timeout="${TIMEOUT_SECONDS}" \
          model.path="${model_path}" \
          rollout.gpu_memory_utilization="${gpu_util}" \
          rollout.temperature=1 \
          rollout.max_model_len="${MAX_MODEL_LEN}" \
          rollout.max_tokens="${MAX_TOKENS_PER_TURN}" \
          rollout.max_num_seqs="${max_num_seqs}" \
          rollout.max_num_batched_tokens="${max_num_batched_tokens}" \
          rollout.tensor_model_parallel_size=1 \
          rollout.rollout_log_dir="${run_dir}/executer_logs" \
          trainer.nnodes=1 \
          trainer.n_gpus_per_node="${NUM_GPUS}"
    ) > "${attempt_log}" 2>&1

    cat "${attempt_log}" >> "${log_path}"
    if grep -q "METRICS_JSON:" "${attempt_log}"; then
      ok=0
      break
    fi
    if is_oom_log "${attempt_log}"; then
      echo "Attempt ${attempt} OOM, fallback to lower concurrency..." | tee -a "${log_path}"
      continue
    fi
    echo "Attempt ${attempt} failed for non-OOM reason, try next profile..." | tee -a "${log_path}"
  done

  return "${ok}"
}

MODEL_LABELS=()
MODEL_PATHS=()

if [[ "${INCLUDE_BASE_MODELS}" == "1" ]]; then
  MODEL_LABELS+=("base_3b")
  MODEL_PATHS+=("${BASE_MODEL_3B}")
  MODEL_LABELS+=("base_7b")
  MODEL_PATHS+=("${BASE_MODEL_7B}")
fi

mapfile -t CKPTS_3B < <(find "${CKPT_ROOT_3B}" -maxdepth 1 -type d -name 'global_step_*' | sort -V)
mapfile -t CKPTS_7B < <(find "${CKPT_ROOT_7B}" -maxdepth 1 -type d -name 'global_step_*' | sort -V)

for ckpt_dir in "${CKPTS_3B[@]}"; do
  MODEL_LABELS+=("3b_$(basename "${ckpt_dir}")")
  MODEL_PATHS+=("${ckpt_dir}/actor/huggingface")
done

for ckpt_dir in "${CKPTS_7B[@]}"; do
  MODEL_LABELS+=("7b_$(basename "${ckpt_dir}")")
  MODEL_PATHS+=("${ckpt_dir}/actor/huggingface")
done

for idx in "${!MODEL_LABELS[@]}"; do
  model_label="${MODEL_LABELS[$idx]}"
  model_path="${MODEL_PATHS[$idx]}"

  if [[ "$(should_skip_model "${model_label}" "${RESULTS_FILE}")" == "1" ]]; then
    echo "===== EVAL SKIP model=${model_label} (already in ${RESULTS_FILE}) ====="
    continue
  fi

  run_name="eval_webshop_${model_label}_$(date -u +%Y%m%d_%H%M%S)"
  run_dir="${ROOT}/runlogs/${run_name}"
  log_path="${run_dir}/eval.log"
  metrics_json_path="${run_dir}/metrics.json"
  merge_log="${run_dir}/merge.log"
  mkdir -p "${run_dir}"

  echo "===== EVAL START model=${model_label} =====" | tee -a "${log_path}"
  if [[ ! -d "${model_path}" ]]; then
    echo "Missing model directory: ${model_path}" | tee -a "${log_path}"
    continue
  fi
  if ! ensure_model_weights "${model_label}" "${model_path}" "${merge_log}"; then
    echo "===== EVAL FAIL model=${model_label}: merge/check failed =====" | tee -a "${log_path}"
    continue
  fi

  if ! run_one_model_with_fallback "${model_label}" "${model_path}" "${run_dir}" "${log_path}"; then
    echo "===== EVAL FAIL model=${model_label}: all attempts failed =====" | tee -a "${log_path}"
    continue
  fi

  python - "${log_path}" "${metrics_json_path}" <<'PY'
import json
import sys
log_path, out_path = sys.argv[1], sys.argv[2]
metrics_line = None
with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
    for line in f:
        if "METRICS_JSON:" in line:
            metrics_line = line.split("METRICS_JSON:", 1)[1].strip()
if metrics_line is None:
    raise SystemExit(f"Failed to find METRICS_JSON in log: {log_path}")
metrics = json.loads(metrics_line)
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(metrics, f, ensure_ascii=True, sort_keys=True)
PY

  ts="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  read -r score succ <<< "$(python - "${metrics_json_path}" <<'PY'
import json
import sys
metrics = json.load(open(sys.argv[1], "r", encoding="utf-8"))
overall = metrics.get("overall", {})
print(f"{float(overall.get('score', 0.0))} {float(overall.get('succ', 0.0))}")
PY
)"

  printf '{"timestamp":"%s","task":"webshop","split":"test","model_label":"%s","model_path":"%s","score":%s,"succ":%s,"log_path":"%s"}\n' \
    "${ts}" "${model_label}" "${model_path}" "${score}" "${succ}" "${log_path}" >> "${RESULTS_FILE}"

  echo "===== EVAL DONE model=${model_label} score=${score} succ=${succ} =====" | tee -a "${log_path}"
done

echo "All WebShop checkpoint evaluations completed."
echo "Results file: ${RESULTS_FILE}"
python - "${RESULTS_FILE}" <<'PY'
import json
import sys
rows = [json.loads(line) for line in open(sys.argv[1], "r", encoding="utf-8") if line.strip()]
print("===== SUMMARY =====")
for row in rows:
    print(
        f"{row.get('model_label', 'unknown'):>24} | "
        f"Score={float(row.get('score', 0.0)):.4f} | "
        f"Succ={float(row.get('succ', 0.0)):.4f}"
    )
PY
